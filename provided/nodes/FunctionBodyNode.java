...
public class FunctionBodyNode implements JottTree {
	...
	public boolean validateTree(SemanticContext context) {
		...
		for (VariableDecNode declaration : variableDeclarations) {
			if (!declaration.validateTree(context)) {
				ok = false;
			}
			if (context.hasError()) {
				return false;
		...
		ValidationSummary summary = validateBlock(context, statements, returnNode);
		alwaysReturns = summary.returns;
		return ok && summary.valid && !context.hasError();
	}

	static ValidationSummary validateBlock(SemanticContext context, List<StatementNode> statements, ReturnNode returnNode) {
		boolean valid = true;
		boolean fallsThrough = true;

		for (StatementNode statement : statements) {
			if (!statement.validateTree(context)) {
				valid = false;
			}
			if (context.hasError()) {
		...
		if (returnNode != null) {
			if (!returnNode.validateTree(context)) {
				valid = false;
			}
			if (!context.hasError()) {
				fallsThrough = false;
		...
		return new ValidationSummary(valid, !fallsThrough);
	}
	...
	static class ValidationSummary {
		...
		ValidationSummary(boolean valid, boolean returns) {
			this.valid = valid;
