...
public class IfNode implements JottTree {
	...
	public boolean validateTree(SemanticContext context) {
		...
		if (!context.hasError() && condition.getType() != JottType.BOOLEAN) {
			context.reportSemanticError("If condition must be Boolean", condition.getToken());
			return false;
		}

		VariableTable variables = context.variables();
		variables.pushScope();
		FunctionBodyNode.ValidationSummary bodySummary = FunctionBodyNode.validateBlock(context, bodyStatements, bodyReturn);
		variables.popScope();
		...
		for (ElseIfClause clause : elseIfClauses) {
			...
			if (!context.hasError() && clause.condition.getType() != JottType.BOOLEAN) {
				context.reportSemanticError("Elseif condition must be Boolean", clause.condition.getToken());
				return false;
			}

			variables.pushScope();
			FunctionBodyNode.ValidationSummary clauseSummary = FunctionBodyNode.validateBlock(context, clause.statements, clause.returnNode);
			variables.popScope();
			ok &= clauseSummary.valid;
			if (!clauseSummary.returns) {
		...
		if (elseClause != null) {
			variables.pushScope();
			FunctionBodyNode.ValidationSummary elseSummary = FunctionBodyNode.validateBlock(context, elseClause.statements, elseClause.returnNode);
			variables.popScope();
			ok &= elseSummary.valid;
			elseReturns = elseSummary.returns;
		} else {
			allElseIfReturn = false;
		}
