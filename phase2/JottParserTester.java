...
public class JottParserTester {
    ...
    private boolean parserTest(TestCase test, String orginalJottCode){
        try {
            ArrayList<Token> tokens = JottTokenizer.tokenize("phase2/parserTestCases/" + test.fileName);
            ...
            try {
                FileWriter writer = new FileWriter("phase2/parserTestCases/parserTestTemp.jott");
                if (jottCode == null) {
                    System.err.println("\tFailed Test: " + test.testName);
            ...
            ArrayList<Token> newTokens = JottTokenizer.tokenize("phase2/parserTestCases/parserTestTemp.jott");
    ...
    private boolean runTest(TestCase test){
        ...
        try {
            orginalJottCode = new String(
                    Files.readAllBytes(Paths.get("phase2/parserTestCases/" + test.fileName)));
        } catch (IOException e) {
            e.printStackTrace();
            return false;
        }
